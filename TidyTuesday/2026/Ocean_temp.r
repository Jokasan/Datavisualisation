...
plot_data <- ocean_temperature %>%
  ...
  mutate(
    day_of_year = as.Date(paste0("2000-", format(date, "%m-%d")), format = "%Y-%m-%d"),
...
heatmap <-ggplot(plot_data,
  ...
  scale_fill_gradientn(
    colours = c("#1B2A49", "#1D6996", "#73AF48", "#EDAD08", "#E17C05", "#CC503E"),
    name    = "Mean Temp (°C)",
    breaks  = seq(0, 25, 5),
  ...
  theme(
    ...
    plot.title           = element_text(family = "domine", size = 18,
    ...
    strip.text.x.top     = element_text(angle = 0, face = "bold", size = 12,
                                         family = "rethink", colour = "#2C3E50",
                                         hjust = 0),
